...
<!--
  ...
  during the workflow's start command (`/start-issue` or `/start-todo`) plan
  step, before any code is written. It checks the proposed plan against the
  rules below and hands back violations plus architecture-preserving
  alternatives.
