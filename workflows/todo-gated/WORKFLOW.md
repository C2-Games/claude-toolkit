...
- **Lint and tests** then run (the exact commands are configured in
  `.claude/commands/check.md` by `INIT.md`). Any failure aborts the change until
  fixed. On pass it stamps a source-tree hash to `.claude/.last-check`.
...
`.claude/commands/`, `.claude/hooks/`, `.claude/agents/`, `.claude/scripts/`,
`.claude/ARCHITECTURE.md`, plus any project paths INIT added) and, when any of those
changed but `.claude/CLAUDE.md` did not, blocks the stop with the list. It fires
once per distinct set of changes, recorded in `.claude/.doc-drift-ack` (gitignored).
If the repo has no `.claude/CLAUDE.md`, the hook does nothing.
