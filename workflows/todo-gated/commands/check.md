...
> `/finish` step 2 exactly — both hash the same file set.
...
Once everything passes, stamp it so `/finish` can confirm freshness later:
...
```bash
git diff origin/main...HEAD -- <!-- INIT: source globs -->
...
significant, `/check` has passed — proceed to `/finish`. If it reports findings,
`/check` has *not* passed: re-enter plan mode (`EnterPlanMode`) to plan the
fixes (the small-change exemption in `/start-todo` still applies if a finding is
...
`/check` succeeds once source changed; `/finish` does not verify this itself, so
