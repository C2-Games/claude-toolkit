...
def _claude_dir() -> Path:
    """The consuming repo's .claude/ directory.

    `scripts/` is a symlink into the shared store, so __file__ points at the
    store, not the project -- never resolve todos.json relative to it. Prefer
    $CLAUDE_PROJECT_DIR (always set when a hook or command invokes this); fall
    back to a `.claude/` in the current directory (a direct CLI run from a repo
    root).
    """
    env = os.environ.get("CLAUDE_PROJECT_DIR")
    base = Path(env).resolve() if env else Path.cwd()
    return base / ".claude"


TODOS_PATH = _claude_dir() / "todos.json"
CURRENT_TODO_PATH = _claude_dir() / ".current-todo"
