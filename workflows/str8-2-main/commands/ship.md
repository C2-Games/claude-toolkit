...
1. **Confirm `/check` is current.** Recompute the same hash `/check` stamps -- use
   the identical find globs from `/check`:
...
2. **Verify the branch.** Confirm `git rev-parse --abbrev-ref HEAD` is the default
   branch -- `main`, or whatever `INIT.md` set. That is expected here; this
   workflow does not use work branches. If HEAD is on some other branch, say so
